...
import { CheckSquare, ChevronDown, Loader2, Square, X } from 'lucide-react';
...
interface UnifiedBulkActionBarProps {
  ...
  isLoading: boolean;
  entityName?: string; // e.g., "RPD", "レビューセット"
}

/**
 * Unified bulk action bar component for consistent bulk operations across different entity types
 */
export default function UnifiedBulkActionBar({
  selectedCount,
  totalCount,
  onSelectAll,
  onDeselectAll,
  ...
  isLoading,
  entityName = 'アイテム',
}: UnifiedBulkActionBarProps): JSX.Element {
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button
                          ...
                          aria-label={`${selectedCount}件の選択済み${entityName}に対する一括操作`}
                        >
                          {isLoading ? (
                            <Loader2 className="h-3 w-3 mr-1 animate-spin" />
                          ) : (
                            bulkActions[0]?.icon || <div className="h-3 w-3 mr-1" />
                          )}
