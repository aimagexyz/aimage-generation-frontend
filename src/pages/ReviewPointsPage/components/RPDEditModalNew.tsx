...
import { Check, Image as RefreshCw, Undo2, Wand2, X } from 'lucide-react';
...
import { NG_SUBCATEGORIES, NG_SUBCATEGORY_INFO, type NGSubcategory, type PredefinedRPDKey } from '@/constants/rpdKeys';
import type { AutofillSuggestion } from '@/types/autofill';

import type { ReviewPointDefinitionVersionBase } from '../../../types/ReviewPointDefinition';
...
import { RPD_KEY_INFO } from './shared/constants';
import { ImageUploadArea } from './shared/ImageUploadArea';
import { RightPanelTabs } from './shared/RightPanelTabs';
import type { AppellationFileInfo, ImageInfo, SpecialRule } from './shared/types';
...
// フォームスキーマ（编辑模式不需要key验证）
const rpdEditSchema = z.object({
  title: z.string().min(1, 'タイトルは必須です').max(255),
  user_instruction: z.string().min(1, '内容は必須です').max(5000),
});

type FormData = z.infer<typeof rpdEditSchema>;

interface RPDEditModalNewProps {
  rpdId: string | null;
  isOpen: boolean;
  onClose: () => void;
  projectId: string;
...
export default function RPDEditModalNew({ rpdId, isOpen, onClose, projectId, onSuccess }: RPDEditModalNewProps) {
  const [currentStep, setCurrentStep] = useState<1 | 2>(2); // 直接进入第二步
  ...
  const [ngSubcategory, setNgSubcategory] = useState<NGSubcategory | null>(null);
  ...
  const createRPDVersionMutation = useCreateRPDVersion({
    onSuccess: async () => {
      // 调用父组件传入的成功回调，用于同步selectedRPD状态
      if (onSuccess) {
        await onSuccess();
      }
    },
  ...
  const [testHistory, setTestHistory] = useState<unknown[]>([]);
  ...
  useEffect(() => {
    if (isOpen && !sessionId) {
      const newSessionId = `rpd-edit-${Date.now()}-${crypto.randomUUID().slice(0, 8)}`;
      setSessionId(newSessionId);
    }
  }, [isOpen, sessionId]);

  // 处理参考图片的辅助函数
  const handleReferenceImages = useCallback((currentVersion: Record<string, unknown>, rpdKey: string) => {
    const referenceImages = currentVersion.reference_images as string[] | undefined;
    if (rpdKey !== 'text_review' && referenceImages && referenceImages.length > 0) {
      const existingImages: ImageInfo[] = referenceImages.map((s3Path: string, index: number) => ({
        id: `existing-${index}`,
  ...
  // 处理称呼表文件的辅助函数
  const handleAppellationFiles = useCallback((versionWithFiles: Record<string, unknown>) => {
    const referenceFiles = versionWithFiles.reference_files as string[] | undefined;
    if (referenceFiles && referenceFiles.length > 0) {
      const s3Path = referenceFiles[0];
      const fileName = s3Path.split('/').pop() || 'appellation_file';
      const isExcel = fileName.toLowerCase().includes('.xls');

      const existingAppellationFile: AppellationFileInfo = {
        id: `existing-appellation-${Date.now()}`,
      ...
      setAppellationFile(existingAppellationFile);
    }
  }, []);

  // 处理特殊规则的辅助函数
  const handleSpecialRules = useCallback((versionWithFiles: Record<string, unknown>) => {
    const specialRulesData = versionWithFiles.special_rules as Array<Record<string, unknown>> | undefined;
    if (Array.isArray(specialRulesData)) {
      const rules: SpecialRule[] = specialRulesData.map((rule: Record<string, unknown>) => ({
        speaker: (rule.speaker as string) || '',
        target: (rule.target as string) || '',
        alias: (rule.alias as string) || '',
  ...
  // 初始化表单数据
  useEffect(() => {
    if (!currentRPD?.current_version || !isOpen) {
      return;
    }
    ...
    // 设置基本数据
    setSelectedKey(currentRPD.key as PredefinedRPDKey);
    setTitle(currentVersion.title || '');
    setDetailContent(String(currentVersion.user_instruction || currentVersion.description_for_ai || '')); // 优先显示用户输入，fallback到description_for_ai
    setTags(currentVersion.tag_list || []);

    // 处理general_ng_review专用数据
    if (currentRPD.key === 'general_ng_review') {
      const versionData = currentVersion as unknown as Record<string, unknown>;
      const subcategory = versionData.ng_subcategory as string | null;
      // 如果ng_subcategory为null或undefined，默认设置为'concrete_shape'
      setNgSubcategory((subcategory as NGSubcategory) || 'concrete_shape');
    }

    // 处理参考图片
    handleReferenceImages(currentVersion as unknown as Record<string, unknown>, currentRPD.key);

    // 处理text_review专用数据
    if (currentRPD.key === 'text_review') {
      const versionWithFiles = currentVersion as unknown as Record<string, unknown>;
      handleAppellationFiles(versionWithFiles);
      handleSpecialRules(versionWithFiles);
    }
  }, [currentRPD, isOpen, handleReferenceImages, handleAppellationFiles, handleSpecialRules]);
  ...
  const form = useForm<FormData>({
    resolver: zodResolver(rpdEditSchema),
    defaultValues: {
      title: '',
      user_instruction: '',
    },
  ...
  const validateBasicFields = (): boolean => {
    if (!rpdId || !currentRPD || !selectedKey || !title.trim()) {
      toast({
        title: 'エラー',
  ...
  // 构建text_review更新数据
  const buildTextReviewData = (): ReviewPointDefinitionVersionBase => {
    const referenceFiles =
      appellationFile && appellationFile.status === 'completed' && appellationFile.s3Url ? [appellationFile.s3Url] : [];
    ...
    return {
      ...
      special_rules: specialRules.length > 0 ? specialRules : undefined,
    } as ReviewPointDefinitionVersionBase;
  };

  // 构建常规RPD更新数据
  const buildRegularRPDData = (): ReviewPointDefinitionVersionBase => {
    const referenceImages = uploadedImages
      .filter((img) => img.status === 'completed')
      .map((img) => img.s3Path || img.s3Url)
      .filter((url): url is string => !!url);

    const baseData: ReviewPointDefinitionVersionBase = {
      title,
      user_instruction: detailContent, // 使用user_instruction而不是description_for_ai
    ...
    // 如果是general_ng_review，添加ng_subcategory
    if (selectedKey === 'general_ng_review' && ngSubcategory) {
      return {
        ...baseData,
        ng_subcategory: ngSubcategory,
      };
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const updateData = selectedKey === 'text_review' ? buildTextReviewData() : buildRegularRPDData();

      await createRPDVersionMutation.mutateAsync({
        rpdId: rpdId!,
        data: updateData,
      });
      ...
      toast({
        title: '成功',
        description: 'RPDの新しいバージョンが正常に作成されました。',
      });

      handleClose();
    } catch (error) {
      console.error('RPD更新エラー:', error);
      toast({
        title: 'エラー',
        description: 'RPDの更新に失敗しました。',
        variant: 'destructive',
      });
    }
  };
    ...
    (imageId: string) => {
      ...
      if (image) {
        ...
        if (image.status === 'completed') {
          ...
          // 现有图片如果没有描述，显示提示信息
          if (image.isExisting) {
            return '既存の画像です。必要に応じて新しい説明を生成できます。';
          }
        }
      }
      return '画像の詳細な説明を生成中です...';
  ...
  const handleRegenerateDescription = useCallback(
    async (imageId: string) => {
      ...
      // 确定使用哪个URL进行描述生成
      const imageUrl = image.s3Url || image.s3Path;
      if (!imageUrl) {
        return;
      }
        ...
        <DialogContent className="sm:max-w-6xl max-h-[85vh] p-0 overflow-hidden border-0 shadow-2xl">
          <div className="flex items-center justify-center py-16">
            <div className="text-center">
              ...
              <p className="text-gray-600">RPD情報を読み込み中...</p>
            </div>
          </div>
        </DialogContent>
      </Dialog>
    ...
    return (
      <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
        <DialogContent className="sm:max-w-6xl max-h-[85vh] p-0 overflow-hidden border-0 shadow-2xl">
          <div className="flex items-center justify-center py-16">
            <div className="text-center">
              <p className="text-red-600">RPD情報が見つかりません</p>
            </div>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <DialogContent className="sm:max-w-6xl max-h-[85vh] p-0 overflow-hidden border-0 shadow-2xl">
        <DialogHeader className="px-6 pt-4 pb-1 bg-gradient-to-r from-slate-50 to-gray-50 flex-shrink-0">
          <div>
            <DialogTitle className="text-xl font-bold text-gray-900 mb-1">
              レビューポイント定義を編集 - {RPD_KEY_INFO[selectedKey!]?.name || selectedKey}
            </DialogTitle>
            <p className="text-sm text-gray-500">
              現在のバージョン: v{currentRPD.current_version_num} • 新しいバージョンを作成します
            </p>
          </div>
        </DialogHeader>
        ...
        <div className="flex h-[calc(85vh-5rem)] overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      {selectedKey === 'general_ng_review' && ngSubcategory && (
                        <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-green-100 text-green-800">
                          {NG_SUBCATEGORY_INFO[ngSubcategory]?.name}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="h-16 flex-shrink-0">
                    <div className="bg-white rounded-lg shadow-sm border border-gray-100 h-full overflow-hidden">
                      <div className="p-1 h-full">
                        {selectedKey === 'general_ng_review' && (
                          <div className="text-xs text-blue-600 mb-1 font-medium">NG監修時はタグの追加を推奨します</div>
                        )}
          ...
          <RightPanelTabs
            ...
            isSubmitting={createRPDVersionMutation.isPending}
          />
        </div>
      </DialogContent>
    </Dialog>
